...
//    private static final int GESTURE_MODE_ONLY_CALENDAR = 1;
    ...
    public int getmCalendarShowMode() {
        return mCalendarShowMode;
    }

    public void setmCalendarShowMode(int mCalendarShowMode) {
        this.mCalendarShowMode = mCalendarShowMode;
    }
...
//        }else {
//            mGestureMode = array.getInt(R.styleable.CalendarLayout_gesture_mode, GESTURE_MODE_DISABLED);
//        }
    ...
    final void setup(CalendarViewDelegate delegate) {
        ...
        initCalendarPosition(delegate.mSelectedCalendarviewCalendar.isAvailable() ?
                delegate.mSelectedCalendarviewCalendar :
     ...
     * @param cur 当前日期时间
     */
    private void initCalendarPosition(CalendarviewCalendar cur) {
    ...
    void updateContentViewTranslateY() {
        CalendarviewCalendar calendarviewCalendar = mDelegate.mIndexCalendarviewCalendar;
        if (mDelegate.getMonthViewShowMode() == CalendarViewDelegate.MODE_ALL_MONTH) {
            mContentViewTranslateY = 5 * mItemHeight;
        } else {
            mContentViewTranslateY = CalendarUtil.getMonthViewHeight(calendarviewCalendar.getYear(), calendarviewCalendar.getMonth(), mItemHeight, mDelegate.getWeekStart())
                    - mItemHeight;
        }
        //已经显示周视图，则需要动态平移contentView的高度
        if (getmWeekPager().getVisibility() == VISIBLE) {
     ...
     */
     final void updateCalendarItemHeight() {
        ...
        if (mContentView == null)
            return;
        CalendarviewCalendar calendarviewCalendar = mDelegate.mIndexCalendarviewCalendar;
        updateSelectWeek(CalendarUtil.getWeekFromDayInMonth(calendarviewCalendar, mDelegate.getWeekStart()));
        if (mDelegate.getMonthViewShowMode() == CalendarViewDelegate.MODE_ALL_MONTH) {
            mContentViewTranslateY = 5 * mItemHeight;
        } else {
            mContentViewTranslateY = CalendarUtil.getMonthViewHeight(calendarviewCalendar.getYear(), calendarviewCalendar.getMonth(),
...
//                }
    ...
    public WeekViewPager getmWeekPager() {
        if (mWeekPager==null) onFinishInflate();
        return mWeekPager;
    }
}
