...
public final class DefaultMonthView extends MonthView {
    ...
    protected void onDrawScheme(Canvas canvas, CalendarviewCalendar calendarviewCalendar, int x, int y) {
        mSchemeBasicPaint.setColor(calendarviewCalendar.getSchemeColor());

        canvas.drawCircle(x + mItemWidth - mPadding - mRadio / 2, y + mPadding + mRadio, mRadio, mSchemeBasicPaint);

        canvas.drawText(calendarviewCalendar.getScheme(),
                x + mItemWidth - mPadding - mRadio / 2 - getTextWidth(calendarviewCalendar.getScheme()) / 2,
    ...
    protected void onDrawText(Canvas canvas, CalendarviewCalendar calendarviewCalendar, int x, int y, boolean hasScheme, boolean isSelected) {
        ...
        if (isSelected) {
            canvas.drawText(String.valueOf(calendarviewCalendar.getDay()), cx, mTextBaseLine + top,
                    mSelectTextPaint);
            canvas.drawText(calendarviewCalendar.getLunar(), cx, mTextBaseLine + y + mItemHeight / 10, mSelectedLunarTextPaint);
        } else if (hasScheme) {
            canvas.drawText(String.valueOf(calendarviewCalendar.getDay()), cx, mTextBaseLine + top,
            ...
            canvas.drawText(calendarviewCalendar.getLunar(), cx, mTextBaseLine + y + mItemHeight / 10,
                    calendarviewCalendar.isCurrentDay() ? mCurDayLunarTextPaint : mSchemeLunarTextPaint);
        } else {
            canvas.drawText(String.valueOf(calendarviewCalendar.getDay()), cx, mTextBaseLine + top,
                    calendarviewCalendar.isCurrentDay() ? mCurDayTextPaint :
                            calendarviewCalendar.isCurrentMonth() ? mCurMonthTextPaint : mOtherMonthTextPaint);
            canvas.drawText(calendarviewCalendar.getLunar(), cx, mTextBaseLine + y + mItemHeight / 10,
                    calendarviewCalendar.isCurrentDay() ? mCurDayLunarTextPaint :
                            calendarviewCalendar.isCurrentMonth() ? mCurMonthLunarTextPaint : mOtherMonthLunarTextPaint);
        }
    }
}
