...
final class LunarCalendar {
     ...
     */
    static void setupLunarCalendar(CalendarviewCalendar calendarviewCalendar) {
        int year = calendarviewCalendar.getYear();
        int month = calendarviewCalendar.getMonth();
        int day = calendarviewCalendar.getDay();
        calendarviewCalendar.setWeekend(CalendarUtil.isWeekend(calendarviewCalendar));
        calendarviewCalendar.setWeek(CalendarUtil.getWeekFormCalendar(calendarviewCalendar));

        CalendarviewCalendar lunarCalendarviewCalendar = new CalendarviewCalendar();
        calendarviewCalendar.setLunarCalendarviewCalendar(lunarCalendarviewCalendar);
        int[] lunar = LunarUtil.solarToLunar(year, month, day);
        lunarCalendarviewCalendar.setYear(lunar[0]);
        lunarCalendarviewCalendar.setMonth(lunar[1]);
        lunarCalendarviewCalendar.setDay(lunar[2]);
        calendarviewCalendar.setLeapYear(CalendarUtil.isLeapYear(year));
        if (lunar[3] == 1) {//如果是闰月
            calendarviewCalendar.setLeapMonth(lunar[1]);
            lunarCalendarviewCalendar.setLeapMonth(lunar[1]);
        }
        ...
        calendarviewCalendar.setSolarTerm(solarTerm);
        calendarviewCalendar.setGregorianFestival(gregorian);
        calendarviewCalendar.setTraditionFestival(festival);
        lunarCalendarviewCalendar.setTraditionFestival(festival);
        lunarCalendarviewCalendar.setSolarTerm(solarTerm);
        if (!TextUtils.isEmpty(solarTerm)) {
            calendarviewCalendar.setLunar(solarTerm);
        } else if (!TextUtils.isEmpty(gregorian)) {
            calendarviewCalendar.setLunar(gregorian);
        } else if (!TextUtils.isEmpty(festival)) {
            calendarviewCalendar.setLunar(festival);
        } else {
            calendarviewCalendar.setLunar(LunarCalendar.numToChinese(lunar[1], lunar[2], lunar[3]));
        }
        lunarCalendarviewCalendar.setLunar(calendarviewCalendar.getLunar());
    }
     ...
     * @return 获取农历节日
     */
    static String getLunarText(CalendarviewCalendar calendarviewCalendar) {
        return getLunarText(calendarviewCalendar.getYear(), calendarviewCalendar.getMonth(), calendarviewCalendar.getDay());
    }
}
