...
public final class WeekViewPager extends ViewPager {
            ...
            public void onPageSelected(int position) {
                ...
                if (view != null) {
                    view.performClickCalendar(mDelegate.getSelectMode() != CalendarViewDelegate.SELECT_MODE_DEFAULT ?
                            mDelegate.mIndexCalendarviewCalendar : mDelegate.mSelectedCalendarviewCalendar, !isUsingScrollToCalendar);
     ...
     * @return 获取当前周数据
     */
    List<CalendarviewCalendar> getCurrentWeekCalendars() {
        List<CalendarviewCalendar> calendarviewCalendars = CalendarUtil.getWeekCalendars(mDelegate.mIndexCalendarviewCalendar,
                mDelegate);
        mDelegate.addSchemesFromMap(calendarviewCalendars);
    ...
    void updateRange() {
        ...
        updateSelected(calendarviewCalendar, false);
        if (mDelegate.mInnerListener != null) {
            mDelegate.mInnerListener.onWeekDateSelected(calendarviewCalendar, false);
        }
    ...
    void scrollToCalendar(int year, int month, int day, boolean smoothScroll) {
        ...
        updateSelected(calendarviewCalendar, smoothScroll);
        if (mDelegate.mInnerListener != null) {
            mDelegate.mInnerListener.onWeekDateSelected(calendarviewCalendar, false);
        }
        if (mDelegate.mCalendarSelectListener != null) {
     ...
     * 更新任意一个选择的日期
     */
    void updateSelected(CalendarviewCalendar calendarviewCalendar, boolean smoothScroll) {
        int position = CalendarUtil.getWeekFromCalendarStartWithMinCalendar(calendarviewCalendar,
                mDelegate.getMinYear(),
                mDelegate.getMinYearMonth(),
                mDelegate.getMinYearDay(),
                mDelegate.getWeekStart()) - 1;
        ...
        BaseWeekView view = (BaseWeekView) findViewWithTag(position);
        if (view != null) {
            view.setSelectedCalendar(calendarviewCalendar);
            view.invalidate();
        }
    }
    ...
    void updateWeekStart() {
        ...
        isUpdateWeekView = false;
        updateSelected(mDelegate.mSelectedCalendarviewCalendar, false);
    }
    ...
    private class WeekViewPagerAdapter extends PagerAdapter {
        ...
        public Object instantiateItem(ViewGroup container, int position) {
            ...
            view.setup(calendarviewCalendar);
